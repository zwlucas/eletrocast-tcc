...
export async function getVideoComments(videoId: string, maxResults: number = 20): Promise<YouTubeCommentThread[]> {
    try {
        const response = await fetch(
...
export async function getChannelLatestVideoOrLiveStream(channelId: string): Promise<{videoId: string, isLive: boolean}> {
    try {
        ...
        if (liveData.items && liveData.items.length > 0 && liveData.items[0].snippet.liveBroadcastContent === "live") {
            return {
        ...
        if (
            ...
            throw new Error(`Channel uploads playlist not found for channel ID ${channelId}`)
        }
        ...
        if (!playlistData.items || playlistData.items.length === 0) {
            throw new Error(`No videos found in uploads playlist for channel ID ${channelId}`)
        }
