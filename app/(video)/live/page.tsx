...
import {
  ...
  getChannelDetails,
  getVideoComments,
  getChannelLatestVideoOrLiveStream,
  getVideoDetails,
  YouTubeChannel,
...
export default async function Home() {
  try {
    ...
    const { videoId, isLive } = await getChannelLatestVideoOrLiveStream(
      channelId
    );
      ...
      <div className="bg-muted/50 rounded-lg p-4">
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            ...
            {isLive ? "Ao vivo agora" : `Publicado ${publishedDate}`}
          </p>
    ...
    <div className="bg-muted/50 rounded-lg h-[400px] lg:h-[calc(100vh-200px)] flex flex-col lg:w-[400px]">
      <div className="p-4 border-b border-muted flex items-center justify-between">
        <h3 className="font-medium">
          {isLive ? "Chat ao vivo" : `Comentários (${comments.length})`}
        </h3>
        {isLive && <LiveBadge />}
      </div>
          ...
          {comments.length > 0 ? (
              ...
              return (
                <div key={commentThread.id} className="flex items-start gap-2">
                  <Avatar className="w-8 h-8">
                    ...
                    <AvatarFallback>
                      {comment.snippet.authorDisplayName[0]}
                    </AvatarFallback>
                  </Avatar>
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatRelativeTime(comment.snippet.publishedAt)}
                      </p>
                    </div>
                    <p className="text-sm text-muted-foreground">
            ...
            <div className="text-center py-8">
              <p className="text-muted-foreground">
                {isLive
                  ? "Ninguém está conversando no momento. Seja o primeiro a enviar uma mensagem!"
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-background">
      <div className="max-w-md w-full bg-muted/50 rounded-lg p-6 text-center space-y-4">
        ...
        <p className="text-muted-foreground">
          Não foi possível carregar os dados do YouTube. Por favor, tente
